...
- **三种阅读模式**：
  - 滚动模式：连续滚动阅读，按章节懒加载（80% 自动加载下一章，最多缓存 3 章）
  - 单页模式：一页一页翻阅
  - 双页模式：左右两页并排，类似实体书
- **排版设置**：字号（12-36px）、行距（1.2-3.0）
- **主题切换**：暗色 / 亮色 / 护眼三主题
- **章节导航**：侧栏目录，支持 TXT 智能章节分割和 MD 标题层级，点击跳转后侧栏保持打开
- **侧栏可调宽度**：目录/书签/高亮侧栏支持拖拽调整宽度（200-500px）
- **自动进度**：打开时自动恢复上次阅读位置，翻页后 3 秒防抖自动保存
- **书签系统**：添加/删除/跳转书签，侧栏面板管理，防止重复添加
- **高亮标注**：选中文本浮动工具栏，4 色高亮，添加笔记
- **键盘导航**：方向键翻页、PageUp/Down、Home/End、Esc 关闭
- **滚轮分页**：300ms 节流的滚轮翻页
...
| 国际化 | 自研 i18n | Zustand 全局语言状态 + 翻译键 |
